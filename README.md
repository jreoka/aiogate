...
### BASE_URL is also the access lock

`BASE_URL` is not just the URL the gate advertises and builds shareable links
with — while it is set, it is **the only address the gate answers on**. Two
things are checked on every request:

1. **Host.** The name the client asked for must be `BASE_URL`'s host. An
...
`BASE_URL` instead, on gate responses and on proxied manifests/streams.

With `BASE_URL` **unset** there is no lock: the gate builds its URLs from the
request host and answers on any host or origin. So the choice is "one address,
locked" or "any address, unlocked".

Notes and edge cases:

- Use the **gate's** own URL, not the Stremio web origin: whichever origin is
  set is the only one allowed, and the panel is bound by the same rule. If your
  Stremio client is Stremio Web (`web.stremio.com`), its browser requests carry
  that origin and are refused — native apps (no `Origin`/`Referer`) are not.
- Native Stremio apps (desktop/TV/mobile), `curl` and players send no
  `Origin`/`Referer` at all, so they keep working — as long as they use
  `BASE_URL`'s host. Only *other sites* are locked out.
- Browsing the panel or a key URL from another site (a link in webmail or a
  ...
  own URL and key URLs inside Stremio.
...
- Whatever terminates TLS in front of the gate must pass the public name
  through (`Host`, or `X-Forwarded-Host` while `TRUST_PROXY` is on, which is
  the default). A proxy that rewrites `Host` to an internal address *and* sends
  no `X-Forwarded-Host` would make everything 403.
- `/healthz` skips the host check so the container healthcheck, which probes
  `127.0.0.1`, still reports healthy.
- A malformed `BASE_URL` is a fatal boot error rather than a silently disabled
  lock (it would otherwise also produce broken shareable links).
...
| `BASE_URL` | request host | Public URL of the instance — AIOStreams uses it for playback links, and the gate uses it to build shareable key URLs and rewrites. **It is also the access lock**: while set, the gate only answers on that host (alternate domains/IPs get 403) and only browser requests whose `Origin`/`Referer` is that exact origin, with `Access-Control-Allow-Origin` answering with it instead of `*`. Unset = no lock, any host/origin. Native apps/players/curl (no `Origin`/`Referer`) keep working on it; `/healthz` is exempt from the host check |
